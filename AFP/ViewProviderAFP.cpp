...
void ViewProviderAFPInternal::updatePlacement(const Base::Placement& p)
{
...
void ViewProviderAFPInternal::switch_node(bool onoff)
{
    if(onoff)
...
bool ViewProviderAFP::isShow() const
{
...
void ViewProviderAFP::attach(App::DocumentObject* pcFeat)
{
    ...
    //now also attach a second visual path to the root for our second AFP element
...
void ViewProviderAFP::update(const App::Property* prop) {
...
void ViewProviderAFP::onChanged(const App::Property* prop)
{   
...
void ViewProviderAFP::upstream_placement(Base::Placement& p, App::GeoFeature* _feat) {
	...
	const std::vector<App::DocumentObject*>& vector = _feat->getInList();
    for(iter it=vector.begin(); it != vector.end(); it++) {
        if((*it)->getTypeId() == App::Part::getClassTypeId()) {
			upstream_placement(p, static_cast<App::Part*>(*it));
            return;
        }
    };
};

void ViewProviderAFP::onSelectionChanged(const Gui::SelectionChanges& msg)
{
...
void ViewProviderAFP::setupContextMenu(QMenu* menu, QObject* receiver, const char* member)
{
